...
#include <OLED/OLED.h>
#include <tuple>
#include "iD3/MP3FileInfo.h"
#include "iD3/id3.h"
#include "iD3/tag.h"
#include <iD3_2/tagHelper.h>
...
std::vector<MP3FileInfo> files2;
//std::vector<tagHelper> files2;
...
void vDirRead2(void *pvParameters)
{
        ...
        if (res == FR_OK) {
            ...
            for (song_info_t file : files) {
                if (strcasecmp(file.lname.c_str(), "") == 0) {
                    //puts("no long name");
                    MP3FileInfo song;
                    song.Init(file.info.fname);
                    if (song.isOK()){
                        files2.push_back(song);
                    }
...
//void vDirRead3(void *pvParameters)
//{
//
//    while (1) {
//        FRESULT res = getFilesFromSD(&files);
...
//                if (strcasecmp(file.lname.c_str(), "") == 0) {
//                    //puts("no long name");
//                    tagHelper song(file.info.fname);
//                    //song.Init(file.info.fname);
//                    if (song.linked()){
//                        files2.push_back(song);
//                    }
...
//                else {
//                    //char *token = strtok((char *) file.lname.c_str(), "-");
//                    tagHelper song((char *)file.lname.c_str());
////                    song.Init(file.lname.c_str());
//                    if (song.linked()){
//                        files2.push_back(song);
//                    }
...
//            puts("finished loading files");
//            int i = 0;
//            for (tagHelper song : files2){
//                printf("track #%d",i++);
//                printf("Artist: %s\n",song.getValue(ID3FID_TITLE));
//                //printf("Track: %s\n",song.szTitle);
//                puts("\n");
//                vTaskDelay(500);
//            }
...
int main(void)
{
    ...
    xTaskCreate(vDirRead2, "DirRead2", 2000, NULL, PRIORITY_LOW, NULL);
    //xTaskCreate(vDirRead3, "DirRead3", 2000, NULL, PRIORITY_LOW, NULL);
    scheduler_start();
    return -1;
}
